...
import it.polimi.se2018.model.objectivecards.PublicObjectiveCard4;
...
public class PublicObjectiveCard4Test {
    ...
    public void testGetPoints() {
        ...
        WindowPattern windowPattern = new PublicObjectiveCard4Test.WindowPatternTest();
        WindowFrame windowFrame = new WindowFrame(windowPattern);

        assertEquals(0, new PublicObjectiveCard4().getPoints(windowFrame));
        ...
        windowFrame.placeDiceUnrestricted(new Dice(Color.RED, 3), new Position(2, 1));
        windowFrame.placeDiceUnrestricted(new Dice(Color.YELLOW, 4), new Position(2, 3));
        windowFrame.placeDiceUnrestricted(new Dice(Color.GREEN, 4), new Position(3, 1));
        assertEquals(0, new PublicObjectiveCard4().getPoints(windowFrame));

        windowFrame.placeDiceUnrestricted(new Dice(Color.BLUE, 1), new Position(0, 1));
        windowFrame.placeDiceUnrestricted(new Dice(Color.PURPLE, 6), new Position(2, 4));
        windowFrame.placeDiceUnrestricted(new Dice(Color.GREEN, 6), new Position(3, 4));
        assertEquals(4, new PublicObjectiveCard4().getPoints(windowFrame));
    }
}
